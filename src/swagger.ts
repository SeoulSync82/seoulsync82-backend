import { CoursePlaceDto } from './course/dto/course.dto';
import { ApiPlaceCultureGetResponseDto } from './place/dto/api-place-culture-get-response.dto';
import { ApiPlaceCultureDetailGetResponseDto } from './place/dto/api-place-culture-detail-get-response.dto';
import { ApiPlaceExhibitionGetResponseDto } from './place/dto/api-place-exhibition-get-response.dto';
import { ApiPlacePopupGetResponseDto } from './place/dto/api-place-popup-get-response.dto';
import { ApiPlaceDetailGetResponseDto } from './place/dto/api-place-detail-get-response.dto';
import { ApiCourseRecommendPostResponseDto } from './course/dto/api-course-recommend-post-response.dto';
import { ApiCourseSubwayCheckGetResponseDto } from './course/dto/api-course-subway-check-get-response.dto';
import { ApiCourseMyHistoryGetResponseDto } from './course/dto/api-course-my-history-get-response.dto';
import { ApiCourseDetailGetResponseDto } from './course/dto/api-course-detail-get-response.dto';
import { ApiCoursePlaceListGetResponseDto } from './course/dto/api-course-place-list-get-response.dto';
import { ApiBookmarkGetResponseDto } from './bookmark/dto/api-bookmark-get-response.dto';
import { ApiBookmarkDetailGetResponseDto } from './bookmark/dto/api-bookmark-detail-get-response.dto';
import { ApiCommunityMyCourseGetResponseDto } from './community/dto/api-community-my-course-get-response.dto';
import { ApiCommunityGetResponseDto } from './community/dto/api-community-get-response.dto';
import { ApiCommunityDetailGetResponseDto } from './community/dto/api-community-detail-get-response.dto';
import { ApiNotificationListGetResponseDto } from './notification/dto/api-notification-list-get-response.dto';
import { ApiSearchGetResponseDto } from './search/dto/api-search-get-response.dto';
import { ApiSearchDetailGetResponseDto } from './search/dto/api-search-detail-get-response.dto';
import { ApiThemeListGetResponseDto } from './theme/dto/api-theme-list-get-response.dto';
import { ApiUserProfileGetResponseDto } from './user/dto/api-user-profile-get-response.dto';
import { ApiCourseSubwayListGetResponseDto } from './course/dto/api-course-subway-list-get-response.dto';

export const SwaggerModels = [
  ApiPlaceCultureGetResponseDto,
  ApiPlaceCultureDetailGetResponseDto,
  ApiPlacePopupGetResponseDto,
  ApiPlaceExhibitionGetResponseDto,
  ApiSearchDetailGetResponseDto,
  ApiSearchGetResponseDto,
  ApiCourseSubwayCheckGetResponseDto,
  ApiCourseRecommendPostResponseDto,
  CoursePlaceDto,
  ApiNotificationListGetResponseDto,
  ApiThemeListGetResponseDto,
  ApiBookmarkGetResponseDto,
  ApiBookmarkDetailGetResponseDto,
  ApiCommunityMyCourseGetResponseDto,
  ApiCommunityGetResponseDto,
  ApiCommunityDetailGetResponseDto,
  ApiCourseMyHistoryGetResponseDto,
  ApiCourseDetailGetResponseDto,
  ApiPlaceDetailGetResponseDto,
  ApiCoursePlaceListGetResponseDto,
  ApiUserProfileGetResponseDto,
  ApiCourseSubwayListGetResponseDto,
];
